#!/usr/bin/env python3
"""
Alternative TTQC script that creates flowing text with mixed formatting
"""
...
class FlowingTimeImageGenerator:
    def __init__(self, output_file="flowing_time_image.png", use_ntp=False, ntp_server=None, use_inky=False):
...
def main():
    parser = argparse.ArgumentParser(description='Generate flowing text image from TTQC_quotes.tsv based on current time')
    parser.add_argument('-o', '--output', default='flowing_time_image.png', help='Output image file name')
    ...
    generator = FlowingTimeImageGenerator(args.output, use_ntp=args.ntp, ntp_server=args.ntp_server, use_inky=args.inky)
    generator.run()
